...
(defn- thread-id [] (Thread/currentThread))
...
(defn min-clients-last-id []
  (do
    (info "query!")
  (if (empty? @clients-last-id) max-id
...
(defn add-log [msg]
  (let [msg (clojure.string/join " " [(thread-id) (System/currentTimeMillis) msg "\r\n"])]
...
(defmethod process-packet ["fetch" "1"] [packet channel]
  (let [last-id (@clients-last-id channel)]
    (do
      (send! channel (json-str {:status "ok" :data (filter (fn [msg] (>= (:id msg) last-id)) @all-msgs)}))
      (swap! clients-last-id assoc channel (+ @max-id 1))
      (clear-old-msgs)
      )
  )
)
...
; default method
(defmethod process-packet :default [packet channel] )
...
(defn -main [& args]
  (run-server (-> #'chartrootm site wrap-request-logging) {:port 9001})
  (info "server started. http://192.168.1.20:9001"))